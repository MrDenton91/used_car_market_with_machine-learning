...
def populate_car_list(zip,page_num):
    with open('./carlist10.csv','a') as f:
...
for zi in zips:
    ...
    if zi >= 50500 and zi <= 55000:
    # at most I'm only allowed to see 50 pages beacuse of the cars.com website, which covers about 11 miles on average
    # not every zip code will yield 50 pages the smallest I've seen is 11 that's broken my code, somre where in the Pensilvania area.
    # I'm implimenting a try except I don't care for the error... this is generaly considered bad practice. 
        for a in range(1,50):
                try:
